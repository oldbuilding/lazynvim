...
-- https://github.com/wookayin/dotfiles/blob/master/nvim/lua/config/pynvim.lua
...
local function echom(msg, hlgroup) vim.api.nvim_echo({ { msg, hlgroup } }, true, {}) end
local function warning(msg) echom(msg, "WarningMsg") end
...
if os.getenv("PYTHON3_HOST_PROG") then vim.g.python3_host_prog = os.getenv("PYTHON3_HOST_PROG") end

-- By default, use python3 w.r.t. $PATH as the host python for neovim.
if vim.g.python3_host_prog == "" or not vim.g.python3_host_prog then vim.g.python3_host_prog = vim.fn.exepath("python3") end
...
local python3_version = setmetatable({ _version = nil }, {
  __call = function(self)
    if self._version then return self._version end
...
local function determine_pip_args()
  ...
  if not has_mac then -- for Linux
    ...
    if py_prefix == "/usr" or py_prefix == "/usr/local" then pip_option = pip_option .. "--user " end
  end
...
local function autoinstall_pynvim()
  if vim.fn.exepath(vim.g.python3_host_prog) == "" then return false end

  -- Ensure pynvim >= 0.4.0.
  local python3_neovim_version = system(assert(vim.g.python3_host_prog) .. " -W ignore -c 'import pynvim; print(pynvim.VERSION.minor)' 2>/dev/null")
  local needs_install = tonumber(python3_neovim_version) == nil or tonumber(python3_neovim_version) < 4
  if not needs_install then return false end
...
local function python3_version_check()
  ...
  if
    py_version and (
      py_version[1] > 3 or py_version[1] == 3 and py_version[2] >= 7 -- requires python 3.7+
    )
  then
    return true
  end
...
if vim.F.npcall(vim.fn.py3eval, "1") ~= 1 then
  ...
  if not py_version then return NO_PYNVIM end
  ...
  vim.schedule(function()
    xpcall(function()
      ...
      for _, augroup in ipairs(groups) do
        if vim.startswith(augroup, "RPC_DEFINE_AUTOCMD_GROUP_") then vim.cmd(([[ autocmd! %s ]]):format(augroup)) end
      end
    end, function(err)
