...
local M = {}
-- https://github.com/wookayin/dotfiles/blob/master/nvim/lua/config/formatting.lua
...
local error_format_command = function(lsp_fallback, range, filetype)
  local msg = string.format(
    ...
    range and "for range formatting on" or "for",
    filetype,
...
function M._setup_command()
  local Format = function(args, lsp_fallback)
    ...
    if not ret then error_format_command(lsp_fallback, range, vim.bo.filetype) end
  end

  local complete = function()
    local bufnr = vim.api.nvim_get_current_buf()
    return vim.tbl_map(function(formatter) return formatter.name end, require("conform").list_formatters(bufnr))
  end

  vim.api.nvim_create_user_command("Format", function(args) return Format(args, true) end, {
    nargs = "*",
    range = true,
    complete = complete,
    desc = "format the current buffer using conform and LSP.",
  })
  vim.api.nvim_create_user_command("ConformFormat", function(args) return Format(args, false) end, {
    nargs = "*",
    range = true,
    complete = complete,
...
function M.create_buf_command(name, formatters)
  ...
  vim.api.nvim_buf_create_user_command(0, name, function(args)
    ...
    if not ret then error_format_command(false, range, vim.bo.filetype) end
  end, {
    range = true,
...
function M._setup_formatexpr()
  ...
  vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function() vim.opt_local.formatexpr = "v:lua.conform_formatexpr()" end,
  })
...
local find_project_root = function(buf)
  ...
  local project_root = (vim.b[buf].project_root or require("utils.path_utils").find_project_root({ ".git" }, { path = path }))
...
function M._should_format_on_save(buf)
  ...
  if project_root:match("/lib/python3.%d+/") then return false end

  local workspace_status = M._workspace_status[project_root]
  if not workspace_status or not workspace_status.enabled then return false end

  if workspace_status.filetypes ~= nil and not vim.tbl_contains(workspace_status.filetypes, vim.bo[buf].filetype) then return false end
...
function M.enable_autoformat(buf, arg, opts)
  ...
  local function echo_status()
    ...
    vim.api.nvim_echo({
      ...
      { ": ", "Normal" },
      { is_enabled and "AutoFormat enabled" or "AutoFormat disabled", is_enabled and "MoreMsg" or "Normal" },
    }, true, {})
  end
  ...
  if arg ~= "status" then
    local msg = ("%s auto-formatting for the project:\n`%s`"):format((M._workspace_status[project_root] or {}).enabled and "Enabled" or "Disabled", project_root)
    local timeout = 1000
    if opts.reason then
      msg = msg .. "\n\nreason:\n" .. opts.reason .. ""
      timeout = 5000
    end
    if opts.format_opts then msg = msg .. "\n\nformat options: " .. vim.inspect(opts.format_opts) .. "" end
...
function M.maybe_autostart_autoformatting(buf, condition)
  ...
  if M._workspace_autostart_checked[project_root] then return true end
