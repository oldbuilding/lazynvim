...
M.toggle_breakpoint = function()
  ...
  if vim.bo.modifiable and vim.bo.modified then vim.cmd([[ silent! noautocmd write ]]) end
end
...
M.toggle_typing = function(name)
  if vim.fn.has("nvim-0.9") == 0 then return vim.api.nvim_err_writeln("toggle_typing: this feature requires neovim 0.9.0.") end
  ...
  ---@type TSNode?
  local type_node = node and vim.tbl_filter(function(n) return n:type() == "type" end, node:named_children())[1] or nil
  ...
  local unpack_generic = function(node)
    ...
    if has_type(node, "generic_type") then
      ...
      if has_type(node, "type_parameter") then
        ...
        if has_type(node, "type") then return node end
      end
    end
    return nil
  end
